...
/*
	Sumowanie dwoch liczb calkowitych
	@param char left[] libcza left
	@param char right[] libcza right
	@param char result[] wynik 
*/
void Sum( char left[], char right[], char result[] ){
	bool minusLeft = false; // znak liczby left
	bool minusRight = false; // znak liczby right
	...
	if( ( (minusRight && !minusLeft) || (!minusRight && minusLeft) ) && ( ( IsBigger(left, right) && IsBigger(right, left ) ) ) ){ // roznica dwoch takich samych liczb
			result[0] = '0';
			result[1] = 0;
		}
	else{
		if( ( minusRight && minusLeft ) || ( !minusRight && !minusLeft ) ){ // dodawanie dwoch liczb tego samego znaku
		Add( left, right, result );
		if( ( minusRight && minusLeft ) ){
			MoveRight( result );
			result[ 0 ] = '-';
			}
		}
		else{ // liczby roznych znakow
