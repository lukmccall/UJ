	...
	if( left[ 0 ] == '0' && right[ 0 ] != '0' ){ // 0/cos
		...
		if( right[ 0 ] == '0')
			...
			if( IsBigger(left, right)) {
				Add(newRight, right, newRight);
				
				while( IsBigger(left,newRight)){
					Add(total, one, total);
					Add(newRight, right, newRight);		
					}
				Delete(newRight, right,newRight);
				if( IsBigger( newRight, left ))
					Delete(newRight,left, left);
				else
					Delete( left, newRight, left);
				}
